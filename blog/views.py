...
class AboutView(TemplateView):
    ...
    # though other pages pull from the base file this category func will not work except we add it to the views
    # handling these pages
...
class PostListView(ListView):
    model = Post
    template_name = 'trial\post_list.html'
    ordering = ['-create_date']
    paginate_by = 10

    # this handles listing of categories on base.html and links the listed categories to the various destination
    # even though other pages pull from the base file this category func will not work except we add it to the
    # views handling these pages
    def get_context_data(self, *args, **kwargs):
        cat_menu = Category.objects.all()
        pop_menu = Post.objects.all()

        context = super(PostListView, self).get_context_data(*args, **kwargs)
        context["cat_menu"] = cat_menu
        context['pop_menu'] = pop_menu
...
def CategoryView(request, cats):
    ...
    cat_menu = Category.objects.all()
    return render(request, 'trial\categories.html',
